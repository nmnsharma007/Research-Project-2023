...
class Machine():
    def __init__(self,P,C,U,S,B,N,Y,R):
        ...
        self.R = R                                      # belief states
        self.lookup = [[] for x in range(self.N)]          # lookup table

    def prune(self,T):
        T_pruned_indices = set()
        for r in self.R:
            pi_r = np.array([[r],[1-r]],dtype=np.float32)
            # print(pi_r)
            temp = [] # store values for given belief state
            for i in range(T.shape[0]):
                gamma = T[i].T
                temp.append(np.matmul(gamma,pi_r).item())
    ...
    def train(self):
        ...
        for k in reversed(range(self.N)):
            ...
            # prune T_k using Lovejoy's algorithm
